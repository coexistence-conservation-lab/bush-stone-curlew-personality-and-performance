...
# m1b: latency to reach food (log:seconds) and survival (yes/no)
ggplot(m1_data, aes(x= survived, y=log(latency), group=survived))+
  geom_boxplot()+
  theme_minimal()

m1b <- glmmTMB(survived ~ log(latency) + (1|bird),
               family = binomial,
               data = drop_na(m1_data))
...
# read in data
m2_data <- left_join(read.csv("survival.csv"), read.csv("latency.csv")) %>%
  clean_names()

# m2a: reach food (yes/no) and persistence (days)
ggplot(m2_data, aes(x=reach, y=persistence, group=reach))+
  geom_boxplot() +
  theme_minimal()

m2a <- glmmTMB(persistence ~ reach + (1|bird),
               family = binomial,
               data = m2_data)

summary(m2a)

# m2b: latency to reach food (log:seconds) and persistence (days)
ggplot(m2_data, aes(persistence, log(latency)))+
  geom_point()+
  geom_smooth(method=lm)+
  theme_minimal()

m2b <- glmmTMB(persistence ~ log(latency), # unsure why here but there are model convergence problems when i include bird as random effect
               data = drop_na(m2_data))
...
m3_data_sum <- left_join(read.csv("distance_daily_mean.csv"),
                     clean_names(read.csv("latency.csv")), by = "bird")

# m3a: reach food (yes/no) and distance moved daily
ggplot(m3_data_sum, aes(x=reach, y=mean_daily_dist, group=reach))+
  geom_boxplot() +
  theme_minimal()

m3a <- glmmTMB(reach ~ dist + (1 | date.x + bird),
               family = binomial,
               data = m3_data)

summary(m3a)

# m3b: latency to reach food and distance moved daily
ggplot(m3_data_sum, aes(mean_daily_dist, latency))+
  geom_point()+
  geom_smooth(method=lm)+
  theme_minimal()

m3b <- glmmTMB(dist ~ log(latency) + (1 | date.x + bird),
               data = drop_na(m3_data))
...
m4b <- glmmTMB(mean ~ log(latency) + (1| date.x + bird),
...
First, test handling response as a predictor for survival
