...
m1b <- glmmTMB(survived ~ latency + (1|bird),
               family = binomial,
...
# hurdle model of reach and distance- cant seem to get this to work
n <- length(unique(m3_data$date.x))
times <- factor(1:n)
group <- factor(rep(1,n))

m3 <- glmmTMB(reach ~ dist + ar1(times) + (1|bird),
              ziformula=~.,   
              family = binomial,
              data = m3_data)
