...
# Bird list excluding disease fates (Wobbles and Star)
birds2 <- c("Briar", "Nutmeg", "Aurora", "Robin",
...
A test case on one bird (Aurora) was used to refine the user input threshold values until visual inspection showed good performance on reducing obvious outliers. We removed duplicate values and fixes taken with less than 4 satellites using the dupfilter from SDLfilter. 

```{r speed filter test}
# Trial with one bird
aurora <- filter(data_raw, id == "Aurora") %>%
  dupfilter(); beep()

speed_filt <- Gundog.GPS.Filter(
  TS = aurora$DateTime,
  Longitude = aurora$lon,
  Latitude = aurora$lat,
...
dupfilter_exact removed 0 of 96898 locations
dupfilter_qi removed 462 of 96898 locations
dupfilter_time removed 0 of 96436 locations
dupfilter_space removed 189 of 96436 locations

Input data: 96898 locations
Filtered data: 96247 locations
dupfilter removed 651 locations (0.67% of original data)
...
# Save to file - column for outliers
write.csv(speed_filt, "Processed/GPSdata_outliers.csv", row.names = FALSE)

# And a smaller version with minimal columns and flagged outliers removed
speed_filt2 <- speed_filt %>%
  filter(Verdict_IF == "Not anomalous") %>%
  select(c("id", "DateTime","lon","lat","acceleration_raw_x", "acceleration_raw_y", "acceleration_raw_z", "external_temperature","height_above_msl","utm_easting","utm_northing","utm_zone"))
...
dupfilter_exact removed 2 of 590860 locations
dupfilter_qi removed 2099 of 590858 locations
dupfilter_time removed 1 of 588759 locations
dupfilter_space removed 924 of 588758 locations

Input data: 590860 locations
Filtered data: 587834 locations
dupfilter removed 3026 locations (0.51% of original data)

And with the gundog.gps.filter
Input data: 587834 locations
Filtered data: 582260
Gundog removed 5574 locations (0.95% of original data)

### Track resampling
The duty cycle for the ornitrack-20 devices included a protocol to reduce tracking time when battery was depeted. As such, fixes were taken at 60, 300, 600, or 3600 seconds, depending on battery. We can resample the tracks to standardise the temporal resolution. Furthermore, resampling will aid in reducing GPS jitter.

```{r resample test subset}
...
robin_amt <- make_track(robin, .x = utm_easting, .y = utm_northing,
                        .t = DateTime, )
```


### Movement verified filtering
Stationary points tend to contribute the most to 'jitter', so checking for real movement can help screen for outliers. 

Movement-verified filtering following (Gunner *_et al._* (2022))[https://royalsocietypublishing.org/doi/10.1098/rsif.2021.0692] where the dynamic body acceleration (DBA; sum of tri-axial accelerometer data) should be in line with velocity (calculated from step distance and time from GPS fixes). 
...
# Plot VeDBA and speed over time (for a 4 hour window)
...
GPS jitter is greater during stationary movement patterns, so I'll run a 2-state state-based model to identify stationary and movement fixes, and then run differential filtering on the two states.
...
# Test subset, 1 month for Nutmeg as a moveHMM object
...
mu0 <- c(1, 25) # step mean (two parameters: one for each state) 
sigma0 <- c(1, 10) # step SD 
...
zone1 <- rbind( # clockwise
  ...
  #c(-37.902573, 144.442970), # Z1/btrw SE corner
  c(-37.902610, 144.443002), # Z1/btrw SE corner
  #c(-37.909514, 144.439601), # Z1 SE corner
...
zone2 <- rbind( #clockwise
  ...
  #c(-37.899903, 144.444270), # Z2/btrw NE corner
...
# Test subset, one bird, one month
aurora <- filter(data, id == "Aurora"
                 & date %in% as_date(c(as_date("2023-03-29"):as_date("2023-04-29"))))

# Convert points to spatial
aurora_sp <- vect(aurora, geom = c("lon", "lat"), crs = "EPSG:4326")
...
ggmap(map_z15) +
  geom_spatvector(data=mtr, inherit.aes = FALSE, alpha = .4,
                  aes(fill = zone))+
  scale_fill_viridis_d()+
  geom_path(data=aurora, aes(lon, lat), inherit.aes = FALSE, colour = "white")+
  theme_void()
```
