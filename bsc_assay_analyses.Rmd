...
### HMM
Last, the majority of remaining jitter is during stationary periods. Here I run a hidden Markov model (HMM) to identify when the bird is stationary (e.g. resting at roost) compared to moving (real movement).  NOTE TO SELF SPLIT DAY NIGHT FIRST
...
mu0 <- c(1, 20, 100) # step mean 
sigma0 <- c(1, 5, 20) # step SD 
stepPar0 <- c(mu0, sigma0) 

angleMean0 <- c(pi, (pi/2), 1) # angle mean 
...
robin_hmm <-fitHMM(robin,
                   ...
                   stepPar0 = stepPar0,
                   anglePar0 = anglePar0,
                   formula = ~vedba) # function of acceleration
...
# Since GPS error is around 50m going to make the 'outside' fence 50m buffer
fence_buff <- fence %>%
  st_as_sf() %>%
  st_union() %>%
  vect() %>%
...
ggmap(map_z15) +
  ...
  scale_fill_viridis_d()+
  geom_path(data=data, aes(lon, lat), inherit.aes = FALSE, colour = "white")+
