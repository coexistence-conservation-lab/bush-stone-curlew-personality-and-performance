...
# Save to file - smaller version with minimal columns and flagged outliers
speed_filt2 <- speed_filt %>%
  select(c("id", "DateTime","lon","lat","acceleration_raw_x", "acceleration_raw_y", "acceleration_raw_z", "external_temperature","height_above_msl","utm_easting","utm_northing","utm_zone", "Verdict", "Verdict_IF"))

write.csv(speed_filt2, "Processed/GPSdata_filtered.csv")
...
dupfilter_exact removed 2 of 1123774 locations
dupfilter_qi removed 2977 of 1123772 locations
dupfilter_time removed 1 of 1120795 locations
dupfilter_space removed 1405 of 1120794 locations

Input data: 1123774 locations
Filtered data: 1119389 locations
dupfilter removed 4385 locations (0.39% of original data)

And with the gundog.gps.filter
Input data: 1,119,389 locations
Filtered data: 1,108,734
Gundog removed 10,655 locations (0.95% of original data)
...
# Read in unfiltered data with outlier annotation
...
data <- read_csv("Processed/GPSdata_filtered.csv") %>%
  mutate(DateTime = as.POSIXct(DateTime, 
                                 "%Y-%m-%d %H:%M:%S",tz = "Australia/Melbourne"),
         date = as.Date(DateTime, tz = "Australia/Melbourne")) %>%
  filter(Verdict_IF == "Not anomalous" & date < as_date("2024-01-10"))
...
ggplot()+
  geom_path(data = smooth, aes(lon, lat, colour = id))+
...
### Manual removal
There are a handful of displacement outliers remaining that I'm going to remove manually based on visual identification. These are for: Briar, Koda, Prem and Robin. I will remove points where the lat/lon exceed simultaneous thresholds. 
...
ggmap(map_z15)+
  geom_path(data=prem, aes(lon, lat), colour = "yellow", inherit.aes = FALSE)+
  theme_minimal()
...
### HMM
Originally did this for filtering but used smoothing instead- keeping for HMM code because the starting parameters seem okay 
