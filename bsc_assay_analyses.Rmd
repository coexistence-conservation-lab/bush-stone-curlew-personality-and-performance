...
We applied filtering in movebank using the 'longest consistent track' approach with a maximum plausible speed of 1.083m/s (3.9 km/h).
```{r movebank filtered}
...
This gives an indiciation of the bird's propensity to explore the release site and exploit new resources. 
...
Out of interest I did this for the filtered data with and without accelerometer filtering to see how much it differed from the filtering done only with SDLfilter. 

```{r distance moved without acc filtering}
...
The paired t.test shows that the two measurements are significantly different. However they are related - with an R^2 value of 0.99. Distance without acc filtering is higher than distance with acc filtering. 

### Weather and distance moved
Added weather to the distance moved graph, to visually inspect any potential correlation. The weather data came from the BOM climate data online
http://www.bom.gov.au/climate/data/ observations were drawn from Avalon Airport {station 087113}.

```{r distance and weather}
# 🚩 Note to self these files are on the other hard-drive 
