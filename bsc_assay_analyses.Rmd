...
```{r traits}
# Read in additional bird trait data
traits <- read_excel("Input/bsc assay data.xlsx", sheet = 8) %>%
  clean_names() %>%
  rename(bird = individual) %>%
  # Group non MR origins into one category
  mutate(origin2 = ifelse(!origin=="Mt Rothwell", "other", "Mt Rothwell"))
```
...
# Weight and tarsus by origin
ggplot(traits, aes(x= tarsus, y= weight, fill = origin2))+
  geom_point()+
  geom_smooth(method = "lm")+
  theme_minimal()
```
...
Score isn't a good predictor of persistence. However, all the score 3 birds survive the full monitoring period. 

Interestingly, the persistence time for score 2 individuals is pretty long (195 & 308 days) - which contradicts my earlier thoughts about score 2 being maladaptive. Indeed, the mean and predicted survival for score 2 is higher than that of score 1. 
...
*Does behaviour predict survival to 90-days post-release?*
...
No evidence that behaviour predicts survival to 90-days post-release. 

### m3b: behaviour ~ survival (1 year)
*Does behaviour predict survival to 1-year post-release?*
...
No evidence that assay behaviour predicts survival to 1-year post-release. 

### m3c: behaviour ~ persistence
*Does behaviour predict persistence?*
