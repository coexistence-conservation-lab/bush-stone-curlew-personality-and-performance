...
* Persistence
* Distance moved
** From release over time (slope)
** Daily movement over time (slope)
* Home range
** 90% overall
** 50% overall
** 90% over time (slope)

And if I get time to add:

* Efficiency
* Sociality

Here we combine the responses into a single table. 
...
Here we model whether latency predicts post-release performance.
...
We use a binomial family glm because the response variable (survival) is binary. Dropping DNRs. Hurdle model - first did they reach the food and then how long it took to reach the food. 
