...
# Load packages
pacman::p_load(adehabitatHR, amt, beepr, ggmap, ggpubr, glmmTMB, gtools, janitor, lme4, move2, progressr, performance, SDLfilter, segclust2d, sf, solitude, suncalc, survival, survminer, readxl, terra, tidyverse, wildlifeDI)
...
m1_data <- survival %>%
  ...
  left_join(latency) %>%
  # Allocate individual bird as factor (to use as random factor)
...
Ideally I should have used bird as a random factor (because some two-thirds of the birds have two latency scores), but the model wasn't able to handle this and gave me an error that "negative log-likelihood is NaN at starting parameter values". 
...
surv <- survfit(Surv(time, status) ~ 1, data = survival2)
plot(surv)
ggcoxzph(surv)
```
